...
//      dtparam=uart1=on                ( append to /boot/firmware/config.txt )
...
//
// based on ttymidi.c by Thiago Teixeira and Jari Suominen - Revision 32
// https://code.launchpad.net/~ttymidi-team/ttymidi/trunk
//-------------------------------------------------------------------------------
...
{
    char *serialdevice_temp = "/dev/ttyAMA4";
    arguments->printonly    = 0;
    arguments->silent       = 0;
    arguments->verbose      = 0;
    arguments->baudrate     = B38400;
    char *name_tmp		    = (char *)"Codex";
    strncpy(arguments->serialdevice, serialdevice_temp, MAX_DEV_STR_LEN);
    strncpy(arguments->name, name_tmp, MAX_DEV_STR_LEN);
}


const char *argp_program_version     = "Codex Midi 1.0";
const char *argp_program_bug_address = "nobody@dev.null";
static char doc[]                    = "Codex Midi - Serial ALSA MIDI Interface";
