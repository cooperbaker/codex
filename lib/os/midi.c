...
//
// based on ttymidi.c by Thiago Teixeira and Jari Suominen (c) 2012 GPL 2.0
// http://www.varal.org/ttymidi/ttymidi.tar.gz
//
// Cooper Baker (c) 2025 GPL 2.0
...
{
	{ "serialdevice" , 's', "DEV" , 0, "Serial device [ /dev/ttyAMA4 ]"     },
	{ "baudrate"     , 'b', "BAUD", 0, "Baud rate [ 38400 ]"                },
	{ "verbose"      , 'v',     0 , 0, "Verbose output"                     },
	{ "printonly"    , 'p',     0 , 0, "Print raw serial data only"         },
	{ "quiet"        , 'q',     0 , 0, "Quiet, print nothing"               },
	{ "name"		 , 'n', "NAME", 0, "Name of Alsa MIDI client [ Codex ]" },
	{ 0 }
};
...
{
	char *serialdevice_temp = DEFAULT_DEVICE;
	arguments->printonly    = 0;
	arguments->silent       = 0;
	arguments->verbose      = 0;
	arguments->baudrate     = B38400;
	char *name_tmp		    = (char *)DEFAULT_NAME;
...
static char doc[]       = "Serial ALSA MIDI Interface";
...
void parse_midi_command(snd_seq_t* seq, int port_out_id, char *buf)
{
    ...
    // buf[1] --> param1
    // buf[2] --> param2   ( not transmitted on program change or key press )
    //
	...
	switch (operation)
	{
		case 0x80:
			...
			break;
		case 0x90:
			if( !arguments.silent && arguments.verbose ) printf( "Serial  0x%x Note on            %03u %03u %03u\n", operation, channel, param1, param2 );
			snd_seq_ev_set_noteon( &ev, channel, param1, param2 );
			break;
		case 0xA0:
			if( !arguments.silent && arguments.verbose ) printf( "Serial  0x%x Pressure change    %03u %03u %03u\n", operation, channel, param1, param2 );
			snd_seq_ev_set_keypress( &ev, channel, param1, param2 );
			break;
		case 0xB0:
			if( !arguments.silent && arguments.verbose ) printf( "Serial  0x%x Controller change  %03u %03u %03u\n", operation, channel, param1, param2 );
			snd_seq_ev_set_controller( &ev, channel, param1, param2 );
			break;
		case 0xC0:
			if( !arguments.silent && arguments.verbose ) printf( "Serial  0x%x Program change     %03u %03u\n", operation, channel, param1 );
			snd_seq_ev_set_pgmchange( &ev, channel, param1 );
			break;
		case 0xD0:
			if( !arguments.silent && arguments.verbose ) printf( "Serial  0x%x Channel change     %03u %03u\n", operation, channel, param1 );
			snd_seq_ev_set_chanpress( &ev, channel, param1 );
			break;
		case 0xE0:
			param1 = ( param1 & 0x7F ) + ( ( param2 & 0x7F ) << 7 );
			if( !arguments.silent && arguments.verbose ) printf( "Serial  0x%x Pitch bend         %03u %05i\n", operation, channel, param1 );
			snd_seq_ev_set_pitchbend( &ev, channel, param1 - 8192 ); // ALSA MIDI needs signed int
			break;
	...
	do
	{
		...
		switch (ev->type)
		{
			...
			case SND_SEQ_EVENT_NOTEOFF:
				bytes[0] = ev->data.control.channel + 0x80;
				bytes[1] = ev->data.note.note;
				bytes[2] = ev->data.note.velocity;
				if( !arguments.silent && arguments.verbose ) printf( "Alsa    0x%x Note off           %03u %03u %03u\n", bytes[0]&0xF0, bytes[0]&0xF, bytes[1], bytes[2] );
				break;

			case SND_SEQ_EVENT_NOTEON:
				bytes[0] = ev->data.control.channel + 0x90;
				bytes[1] = ev->data.note.note;
				bytes[2] = ev->data.note.velocity;
				if( !arguments.silent && arguments.verbose ) printf( "Alsa    0x%x Note on            %03u %03u %03u\n", bytes[0]&0xF0, bytes[0]&0xF, bytes[1], bytes[2] );
				break;
			...
			case SND_SEQ_EVENT_KEYPRESS:
				...
				bytes[1] = ev->data.note.note;
				bytes[2] = ev->data.note.velocity;
				if( !arguments.silent && arguments.verbose ) printf( "Alsa    0x%x Pressure change    %03u %03u %03u\n", bytes[0]&0xF0, bytes[0]&0xF, bytes[1], bytes[2] );
				break;

			case SND_SEQ_EVENT_CONTROLLER:
				bytes[0] = ev->data.control.channel + 0xB0;
				bytes[1] = ev->data.control.param;
				bytes[2] = ev->data.control.value;
				if( !arguments.silent && arguments.verbose ) printf( "Alsa    0x%x Controller change  %03u %03u %03u\n", bytes[0]&0xF0, bytes[0]&0xF, bytes[1], bytes[2] );
				break;

			case SND_SEQ_EVENT_PGMCHANGE:
				bytes[0] = ev->data.control.channel + 0xC0;
				bytes[1] = ev->data.control.value;
				if( !arguments.silent && arguments.verbose ) printf( "Alsa    0x%x Program change     %03u %03u %03u\n", bytes[0]&0xF0, bytes[0]&0xF, bytes[1], bytes[2] );
				break;

			case SND_SEQ_EVENT_CHANPRESS:
				bytes[0] = ev->data.control.channel + 0xD0;
				bytes[1] = ev->data.control.value;
				if( !arguments.silent && arguments.verbose ) printf( "Alsa    0x%x Channel change     %03u %03u %03u\n", bytes[0]&0xF0, bytes[0]&0xF, bytes[1], bytes[2] );
				break;

			case SND_SEQ_EVENT_PITCHBEND:
				bytes[0] = ev->data.control.channel + 0xE0;
				ev->data.control.value += 8192;
				bytes[1] = (int)ev->data.control.value & 0x7F;
				bytes[2] = (int)ev->data.control.value >> 7;
				if( !arguments.silent && arguments.verbose ) printf( "Alsa    0x%x Pitch bend         %03u %5d\n", bytes[0]&0xF0, bytes[0]&0xF, ev->data.control.value );
				break;
...
main(int argc, char** argv)
{
	...
	if (arguments.printonly)
	{
		printf("Printing Raw Serial Data Only...\n");
	}
